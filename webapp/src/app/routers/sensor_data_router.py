...
# PATH DEFINITIONS 
@route.get(path="/api/sensor/latest",
           tags=["Latest Data"],
...
async def get_latest_sensor_data(sensor_id: str) -> SensorDataResponse:
    ...
    if sensor_id == None or len(sensor_id.strip())<1:
        ...
        return JSONResponse(status_code=err.httpCode,
        ...
        try:
            if result.data == None:
                ...
                return JSONResponse(status_code=status.HTTP_404_NOT_FOUND,
                    content=jsonable_encoder(SensorDataResponse(**result.__dict__).dict()))
            else:
            ...
            return JSONResponse(status_code=result.httpCode,
                content=jsonable_encoder(Errors(**result.__dict__).dict()))



@route.post(path="/api/sensor/data",
            tags=["Timed Data"],
